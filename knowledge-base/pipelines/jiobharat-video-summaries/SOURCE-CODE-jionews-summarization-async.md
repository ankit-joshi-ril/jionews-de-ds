# Source Code: thenewj-com/jionews-summarization-async
> Auto-synced from GitHub on 2026-04-14 11:54 UTC
> Branch: `main` | Role: Summarization service (shared, used for JioBharat video summaries)
    ...
    def gemini_call(self, article_url="", article_content="", language_name="", generate_fields="both"):
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                msg = str(e).lower()
                error_message += f"Attempt {attempt} failure_msg: {msg}. "
                ...
                if attempt == self.MAX_RETRIES:
                    error_message = f"Failed after {self.MAX_RETRIES} retries with error messages: {error_message}"
                    return {"title": "", "summary": "", "compliance_score": 0, "error_message": error_message,
...
class TestModelGeneration:
    def __init__(self):
        logger.info("Initializing TestModelGeneration...")
        self.gcp_handler = GCPHandler()
        self.genai_client = self.initialize_genai_client()
        self.default_model = "gemini-2.5-flash"
    ...
    def initialize_genai_client(self):
        ...
        gemini_secret_name = "projects/266686822828/secrets/GEMINI_API_KEY/versions/latest"
        gemini_api_key = self.gcp_handler.get_secret(gemini_secret_name)
        return genai.Client(api_key=gemini_api_key)

    def get_article_content_via_proxy(self, article_url: str) -> str:
        logger.info(f"Fetching article content via proxy for URL: {article_url}")
        base_url = "https://jn-article-render-proxy-266686822828.asia-south1.run.app/proxy"
        proxy_url = f"{base_url}?url={quote(article_url)}"
        ...
        try:
            resp = requests.get(proxy_url, timeout=60)
            resp.raise_for_status()
            logger.info("Successfully fetched article content via proxy.")
            return resp.text
        except requests.RequestException as e:
            logger.error(f"Error fetching article via proxy: {e}")
            return ""

    def gemini_call(self, article_url="", article_content=""):
        logger.info("Starting Gemini model call...")
        ...
        try:
            ...
            logger.error(f"Error during Gemini model call: {e}")
            return ""
    ...
    def test_generate_output(self, article_url):
        logger.info(f"Testing model generation for URL: {article_url}")
        article_content = self.get_article_content_via_proxy(article_url)
        if not article_content:
            logger.warning("Failed to fetch article content. Proceeding with URL only.")
        output = self.gemini_call(article_url=article_url, article_content=article_content)
        logger.info(f"Raw model output: {output}")

        # Parse the output using the safe_parse_llm_json method
        parsed_output = self.safe_parse_llm_json(output)
        if parsed_output:
            title = parsed_output.get("title", "")
            summary = parsed_output.get("summary", "")
            compliance_score = parsed_output.get("compliance_score", 0)
            error_message = parsed_output.get("error_message", "")

            # Calculate word counts
            title_word_count = len(title.split())
            summary_word_count = len(summary.split())

            # Print the parsed fields with word and character counts
            print("Parsed Model Output:")
            print(f"Title Length: {len(title)} characters, {title_word_count} words")
            print(f"Summary Length: {len(summary)} characters, {summary_word_count} words")
            print(f"Title: {title}")
            print(f"Summary: {summary}")
            print(f"Compliance Score: {compliance_score}")
            print(f"Error Message: {error_message}")
        else:
            logger.error("Failed to parse the model output.")
...
class GCPHandler:
    def __init__(self):
        logger.info("Initializing GCPHandler...")
        self.secret_client = secretmanager.SecretManagerServiceClient()
...
tester = TestModelGeneration()
tester.system_instruction = (
    "You are a senior news editor for a reputable, high-traffic digital news outlet.\n"
    "Your responsibility is to generate publishable news content that is highly engaging, "
    "editorially responsible, accurate, and ethical.\n"
    "You act as a news editor/writer and summarize news articles accurately and concisely."
...
test_url = "https://www.republicworld.com/world-news/japan-pm-set-to-dissolve-parliament-calls-for-snap-elections-on-feb-8?utm_source=JioNews&utm_medium=referral&utm_campaign=JioNews"
result = tester.test_generate_output(test_url)
...
def publish_message_to_topic(topic_name, data):
    ...
    message_bytes = json_string.encode("utf-8")
    try:
        result = publisher_client.publish(topic_path, data=message_bytes)
        result.result()  # Wait for the publish call to complete
        print(f"Message published successfully with ID: {result.result()}")
    except Exception as err:
...
try:
    ...
    summaries_hygiene_failure_stg = ingestion_db['summaries_hygiene_failure_stg']
...
results = list(summaries_hygiene_failure_stg.aggregate(pipeline))

print(f"Total {len(results)} records fetched from MongoDB.")

# update sourceid to sourceId + 'batch_test' + incremental count
for idx, record in enumerate(results):
    record['sourceId'] = f"{record.get('sourceId', 'unknown')}_batch_test_{idx + 1}"
# Publish records in batch of 5
batch_size = 5
for i in range(0, len(results), batch_size):
    batch = results[i:i + batch_size]
    print(f"publishing for {len(batch)} Ids:: {[record.get('sourceId', "no_id_found") for record in batch]}")
    publish_message_to_topic("RawSummariesIngestion_HygineFailure_STG", batch)
```
