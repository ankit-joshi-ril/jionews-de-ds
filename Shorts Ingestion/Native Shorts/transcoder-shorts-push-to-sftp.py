...
# Supported resolutions for vertical (portrait) short-form content
# Source: transcoder team spec — files outside these dimensions will be rejected
ALLOWED_RESOLUTIONS = {(1080, 1920), (720, 1280)}
...
class Execute:
    def __init__(self):
        ...
        self.db = MongoDB()
        self.sftp = SFTP()
        self.tmp_dir = 'tmp' if _run_local else '/tmp'
    ...
    def process(self, video_rec):
        ...
        try:
            ...
            upload_status = self.sftp.upload_to_sftp(
                local_video_path,
                f"{SFTP_REMOTE_DIR}/{video_id}.mp4"
            )
            ...
            upload_status = self.sftp.upload_to_sftp(
                local_csv_path,
                f"{SFTP_REMOTE_DIR}/{video_id}.csv"
            )
            if upload_status != 'success':
                video_rec['transcoderProcessingStatus'] = "failed"
                video_rec['errorMessage'] = upload_status
