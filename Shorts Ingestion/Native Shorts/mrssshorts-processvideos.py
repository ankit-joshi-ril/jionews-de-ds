...
class Processor:
    def __init__(self, debug, feeds_config, ps):
        ...
        self.set_name = "de_mrss_shorts_cache"
    ...
    def filter_records_from_cache(self, data, category_id, language_id):
        ...
        for rec in data:
            ...
            # Check if the compound key exists in the ZSet AND has not expired.
            # zscore() returns None when the key is absent; a numeric score otherwise.
            # We treat a score <= current_timestamp as expired (same as not cached).
            ...
            if score is None or score <= current_timestamp:
                ...
                # (Re-)add the compound key with a fresh expiration score
    ...
    def process_mapping(self, record_data, default_values):
        ...
        if source_publisher_id in ["7777", "7778"]:
            ...
            if video_type not in ["shorts", "short", "short video", "shortvideo"]:
                return None
        elif source_publisher_id == "7784":
            if record_data.get('vid_type', '').lower() != 'shorts':
                return None

        source_thumbnail_url = self.get_image_thumbnail_url(record_data)
        if source_publisher_id == "7784":
            vid_thumbnail = record_data.get('vid_thumbnail', '').strip()
            if not vid_thumbnail:
                # Derive YouTube thumbnail from the video ID as fallback
                vid_url = record_data.get('vid_url', record_data.get('link', ''))
                yt_id_match = re.search(r'(?:v=|youtu\.be/)([A-Za-z0-9_-]{11})', vid_url)
                if yt_id_match:
                    vid_thumbnail = f"https://img.youtube.com/vi/{yt_id_match.group(1)}/hqdefault.jpg"
            source_thumbnail_url = vid_thumbnail or source_thumbnail_url
        original_thumbnail_url = source_thumbnail_url
        if source_thumbnail_url:
            ...
            if default_image:
                ...
                # Category not in default image map — use empty thumbnail rather than crashing
                print(f"process_mapping: no default image for category '{source_category_name}' "
                      f"(pub {source_publisher_id}), continuing without thumbnail")
        ...
        final_record = {
            ...
            "sourceThumbnailURL": original_thumbnail_url or source_thumbnail_url,
        ...
        if not RUN_LOCAL:
            ...
            if processed_data:
                ...
                for rec in processed_data:
                    data = [rec]
                    PubSub.publish_message_to_topic("MRSSShortsIngestion_ProcessedData", data)
            else:
