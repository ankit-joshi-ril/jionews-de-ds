...
class Execute:
    ...
    def process_video(self, video_rec):
        ...
        if video_uploaded:
            print(f"{video_id}:: Copying video to transcoder bucket..")
            transcoder_submission = self.gcs.copy_file_to_transcoder(f"{video_id}.mp4")
            if transcoder_submission:
                processing_status["transcoderProcessingStatus"] = "initiated"
            else:
                processing_status["transcoderProcessingStatus"] = "failed"
                processing_status["transcoderErrorMessage"] = f"Failed to copy {video_id}.mp4 to transcoder bucket."
                print(f"{video_id}:: Failed to copy to transcoder bucket.")
    ...
    def run(self, message):
        ...
        source = video_rec.get("src", "")
        # Skip processing for manual uploads
