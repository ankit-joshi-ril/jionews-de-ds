...
class RSS:
    ...
    def epoch_to_ist_iso_safe(self, epoch_seconds=None) -> str:
        """
        Convert epoch timestamp (seconds) to IST ISO format.
        Falls back to current IST time if input is invalid.
        ...
        try:
            # Attempt to parse epoch
            if epoch_seconds is None or epoch_seconds == "":
                raise ValueError("Empty epoch")

            epoch_float = float(epoch_seconds)

            # Optional safety check (avoid absurd values)
            if epoch_float <= 0:
                raise ValueError("Invalid epoch")

            dt_ist = datetime.fromtimestamp(epoch_float, tz=ist)

        except Exception:
            # Fallback to current IST timestamp
            dt_ist = datetime.now(ist)

        # Return fixed format
        return dt_ist.strftime("%Y-%m-%dT%H:%M:%S.000+05:30")
    ...
    def prepare_rss(self, data_list):
        ...
        try:
            for item in data_list:
                ...
                iso_date = self.epoch_to_ist_iso_safe(created_at_epoch)
                ...
                ET.SubElement(item_elem, 'pubDate').text = iso_date
                ...
                media_content_elem.set('duration', item.get('duration', '0'))
                ...
                ET.SubElement(item_elem, 'hlsAvcUrl').text = item['hlsAvcUrl']
                ET.SubElement(item_elem, 'hlsHevcUrl').text = item['hlsHevcUrl']
                ...
                # SourceDate
                ET.SubElement(item_elem, 'sourceDate').text = iso_date
