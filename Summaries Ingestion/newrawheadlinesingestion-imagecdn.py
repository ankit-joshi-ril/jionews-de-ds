...
class CDN:
    ...
    def download(self, url):
        try:
            ...
            # Log non-success responses (especially 403)
            if r.status_code == 406:
                r = requests.get(url,timeout=10,verify=False,headers=REQUEST_HEADERS)

            if r.status_code != 200:
                print(
                    f"Image download HTTP error. URL: {url}, "
            ...
            print(
                f"HTTPError during image download. URL: {url}, "
            ...
            print(f"General exception during image download. URL: {url}, Exception: {e}")
            raise
...
class Execute:
    ...
    def print_current_ip(self):
        """
        Prints current public IP of execution environment.
        Used for debugging when source blocks requests (403 etc).
        """
        try:
            resp = requests.get("https://api.ipify.org?format=json", timeout=5)
            if resp.status_code == 200:
                ip = resp.json().get("ip")
                print(f"Current public IP address: {ip}")
            else:
                print(f"Unable to fetch IP. Status code: {resp.status_code}, Response: {resp.text}")
        except Exception as e:
            print(f"Error fetching current IP address. Exception: {e}")

    def run(self, message):
        try:
            self.print_current_ip()
        except Exception as e:
            print(f"Error printing current IP address: {e}")
