...
def write_to_mongodb(cloud_event):
    ...
    headlines_hygiene_failures_collection = ingestion_db['headlines_hygiene_failures']
    if len(decoded_data) > 0:
        try:
            ins_cur = headlines_hygiene_failures_collection.insert_many(decoded_data, ordered=False)
