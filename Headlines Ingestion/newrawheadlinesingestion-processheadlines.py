...
class GCS:
    def __init__(self, debug):
        ...
        self.file_path = "headlines/headlines_publishers_feeds.csv"
...
class Processor:
    def __init__(self, debug, feeds_config, ps):
        ...
        # self.redis_url = "redis://:af4c82a9-64ee-4757-abe9-cb919feb2942@10.12.16.4:6379"
        ...
        self.title_set_name = "de_headlines_title_cache"
    ...
    def deduplication_title(self, all_records):
        ...
        for rec in all_records:
            ...
            compound_key = re.sub(r'\s+', ' ', title.strip())
            rec['compound_key'] = compound_key

            # Check if the compound key exists in the ZSet
            if not self.redis_client.zscore(self.title_set_name, compound_key):
                ...
                self.redis_client.zadd(self.title_set_name, {compound_key: current_timestamp + self.expiration_seconds})
    ...
    def filter_records_from_cache(self, data, category_id, language_id):
        ...
        # return None
        try:
            # Deduplicate based on title first
            all_records = self.deduplication_title(data)
        except Exception as e:
    ...
    def append_utm_params(self, url, pub_name):
        ...
        # Handle ex_cid for ESPN
        if pub_name.lower() == "espncricinfo":
            if 'ex_cid=' in url:
                url = re.sub(r'ex_cid=[^&]+', 'ex_cid=jionews', url)  # Replace existing ex_cid
            else:
                utm_params += "&ex_cid=jionews"
    ...
    def process_get_article(self, doc):
        ...
        article_text, article_html = self.get_article_text_new(url)
    ...
    def safe_publisher_epoch(self, epoch_now, publisher_epoch):
        try:
            if publisher_epoch and "IST" in publisher_epoch:
                publisher_epoch = publisher_epoch.replace("IST", "+0530")

            parsed_date = Utils.generic_parse_date(publisher_epoch)
            ...
            if date_epoch > epoch_now:
                return epoch_now
            else:
                return date_epoch

        except Exception as e:
            print(f"Exception caught while parsing publisher epoch: {e}")
            return epoch_now
    ...
    def process_mapping(self, record_data, default_values):
        ...
        np_target_publishers = ['english-newspointapp',
                                'Indiatimes',
                                'Navbharat Times', 'Newspoint']

        # AJ- Handler for Newspoint publisher as per STORY 35583 (261124)
        if source_publisher_name in np_target_publishers:
            # source_publisher_name = record_data['pn']
            # List of target Newspoint publishers

            if source_publisher_name not in np_target_publishers:
                return None

            source_publisher_name = 'Newspoint'  # Publication Name Change english-newspointapp to Newspoint (STORY 36532)

            title = record_data['hl']
            source_description = ""
            url = record_data['mwu']
            source_publish_date = record_data['dl']
            np_category_name = record_data['sec'].lower()
            np_category = self.get_np_category(np_category_name)
            source_category_id = str(np_category["id"])
            source_category_name = str(np_category["name"])
        else:
            # ESPNcricinfo RSS feed article URL tag update [STORY 42429]
            if source_publisher_name == "ESPNcricinfo":
                url = record_data['href']
            else:
            ...
            source_description = record_data['summary']
            # url = record_data['link']
            source_publish_date = record_data['published']
        ...
        try:
            publisher_article_body = record_data.get('content', '')[0].get('value', '')
        except Exception as e:
            self.debug(f"Exception caught in extracting publisher_article_body: {e}")
        ...
        if not self.is_within_last_24_hours(source_publish_date):
            return None

        # source_publish_date = self.publisher_date_to_epoch(source_publish_date) or epoch_now

        # source_publish_date = self.adjust_publisher_epoch(epoch_now, source_publish_date)

        source_publish_date = self.safe_publisher_epoch(epoch_now, source_publish_date)

        # Reject headlines with no image (TASK 50615)
        ...
        # PubSub.publish_message_to_topic("NewRawHeadlinesIngestion_image_cdn", cdn_record)

        # Add UTM Param [STORY 26121]
        try:
            url = self.append_utm_params(url, source_publisher_name)
        except Exception as e:
        ...
        self.debug("Check fetch article body")

        # if final_record['sourceLanguageName'] == 'English':
        final_record['articleBody'], final_record['articleHtml'] = self.process_get_article(final_record)
        self.debug("Check fetch article body done")
        self.debug(f"curr final rec: {final_record}")

        # Add default category image if no thumbnail found (BUG-33735)
        if source_thumbnail_url == "":
            # Reject headlines with no image (TASK 50615)
            final_record['processing_status'] = "rejected"
            final_record['error_message'] = "No thumbnail image url found"
        ...
        processed_record = {"filename": source_id, "url": source_thumbnail_url, "category": source_category_name,
                            "publisher": source_publisher_name, "content_type": "headlines", "data": final_record}
...
class Execute:
    ...
    def run(self, message):
        ...
        self.debug(
            f"Total {len(self.processor.non_existing_records)} unique records from {len(headlines_data)} received")
        ...
        # if processed_data:
        #     PubSub.publish_message_to_topic("NewRawHeadlinesIngestion_processed_data", processed_data)
        #     PubSub.publish_message_to_topic("NewRawHeadlinesIngestion_processed_data_STG", processed_data)

        """
        *** Reject headlines with no image (TASK 50615) ***
        Making the workflow synchronous to streamline rejection of headlines without original image asset.
        """
        if processed_data:
